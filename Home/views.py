...
def SignUp(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # ✅ Only send email if the user entered one
            if user.email:
                subject = "Welcome to Recycling E-commerce"
                message = f"Hi {user.username},\n\nThank you for registering with us!\n\nWe’re excited to have you onboard."
                
                send_mail(
                    subject,
                    message,
                    settings.EMAIL_HOST_USER,   # ✅ Use your configured sender email
                    [user.email],
                    fail_silently=False,
                )

            messages.success(request, 'User registered successfully! Please check your email.')
            return redirect('SignIn')
        else:
